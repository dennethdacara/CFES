      ...
      <div class="col-md-8">
        ...
        <div class="card card-default">
          <div class="card-header">
            <h3 class="card-title">Edit Subject: <i>{{$subject->name}}</i></h3>
          </div>
          <form method="POST" action="{{route('subjects.update', ['id' => $subject->id])}}" enctype="multipart/form-data">
            ...
            <div class="card-body">
              ...
              <div class="form-group">
                ...
                <select name="gradelevel_id" id="gradelevel_id" class="form-control">
                  ...
                  @forelse($gradelevels as $gradelevel)
                    <option value="{{$gradelevel->id}}" {{$gradelevel->id == $subject->gradelevel_id ? 'selected' : ''}}>
                      {{$gradelevel->name}}
                    </option>
                  @empty
              ...
              <div class="form-group">
                <label>Code*</label>
                <input type="text" class="form-control" name="code" id="code" placeholder="Enter subject code" value="{{$subject->code}}">
                @if ($errors->has('code'))
              ...
              <div class="form-group">
                <label>Name*</label>
                <input type="text" class="form-control" name="name" id="name" placeholder="Enter subject name" value="{{$subject->name}}">
                @if ($errors->has('name'))
